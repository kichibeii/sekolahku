    ...
    <div class="utama">
      ...
      <div id="main-content">
        ...
        <div class="col-md-9 maindata">
          <form method="POST" action="lihat2.php">
            <select name="id_ag" required>
              <option disabled selected value="">Mata Pelajaran</option>
              <?php 
                $query1=mysqli_query($conn,"SELECT * FROM ambil_guru WHERE id_guru = '$id_guru'");
                while ($data1=mysqli_fetch_array($query1)){
                  $id_mapel=$data1['id_mapel'];
                  if($id_mapel==$buffer){
                    continue;
                  }
                  $query2=mysqli_query($conn, "SELECT * FROM mapel WHERE id_mapel='$id_mapel'");
            ...
            <select name="tahunajaran" required>
              <option disabled="" selected="" value="">Tahun ajaran masuk siswa</option>
              <?php
                while($tahunmulai<=date("Y")-3){
              ?>
              ...
              ?>
              <option style="color:green" value="<?php echo $tahunmulai; ?>"><?php echo $tahunmulai,"/",$tahunmulai+1; $tahunmulai++?> (Aktif)</option>
              <option style="color:green" value="<?php echo $tahunmulai; ?>"><?php echo $tahunmulai,"/",$tahunmulai+1; $tahunmulai++?> (Aktif)</option>
              <option style="color:green" value="<?php echo $tahunmulai; ?>"><?php echo $tahunmulai,"/",$tahunmulai+1; $tahunmulai++?> (Aktif)</option>
            </select>
